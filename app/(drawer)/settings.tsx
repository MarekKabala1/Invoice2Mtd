...
export default function SettingsScreen() {
  ...
  const [taxRates, setTaxRates] = useState<TaxRates>(
    settings ? parseTaxRates(settings.taxRatesJson) : RATES_2025_26
  );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (formState.taxScheme !== settings.taxScheme)
        updatePayload.taxScheme = formState.taxScheme;
      ...
      if (formState.currency !== settings.currency)
        updatePayload.currency = formState.currency;
      if (formState.dateFormat !== settings.dateFormat)
        updatePayload.dateFormat = formState.dateFormat;
      if (formState.numberFormat !== settings.numberFormat)
        updatePayload.numberFormat = formState.numberFormat;
      if (formState.language !== settings.language)
        updatePayload.language = formState.language;
      if (formState.theme !== settings.theme)
        updatePayload.theme = formState.theme;
      if (formState.logoUrl !== settings.logoUrl)
        updatePayload.logoUrl = formState.logoUrl;
  ...
  return (
    <>
      ...
      >
        ...
        {hasChanges && (
          <View
            className="p-3 rounded-lg mb-3 flex-row items-center gap-2"
            style={{ backgroundColor: isDark ? 'rgba(255,193,7,0.1)' : 'rgba(255,193,7,0.2)' }}
          >
            ...
            <SettingsInputRow
              ...
              onChangeText={(text) => {
                ...
                if (!isNaN(num) && num >= 1 && num <= 12)
                  setFormState({ ...formState, financialYearStartMonth: num });
              }}
              placeholder="1"
              keyboardType="numeric"
              maxLength={2}
            ...
            <SettingsInputRow
              ...
              onChangeText={(text) => {
                ...
                if (!isNaN(num) && num >= 1 && num <= 31)
                  setFormState({ ...formState, financialYearStartDay: num });
              }}
              placeholder="1"
              keyboardType="numeric"
              maxLength={2}
          ...
          <View className="flex-1">
            <SettingsInputRow
              ...
              onChangeText={(text) => {
                const num = parseInt(text);
                if (!isNaN(num) && num >= 1 && num <= 12)
                  setFormState({ ...formState, financialYearEndMonth: num });
              }}
              placeholder="12"
              keyboardType="numeric"
              maxLength={2}
          ...
          <View className="flex-1">
            <SettingsInputRow
              ...
              onChangeText={(text) => {
                const num = parseInt(text);
                if (!isNaN(num) && num >= 1 && num <= 31)
                  setFormState({ ...formState, financialYearEndDay: num });
              }}
              placeholder="31"
              keyboardType="numeric"
              maxLength={2}
      ...
      <Modal visible={showUserPicker} animationType="fade" transparent>
        <View className="flex-1" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
          <View
            ...
            style={{ backgroundColor: colors.card, marginTop: 100 }}
          >
