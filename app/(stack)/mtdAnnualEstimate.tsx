...
export default function MtdAnnualEstimateScreen() {
  ...
  const taxEstimate = useMemo(() => {
    ...
    return projectFullYearTax(
      totals.quartersWithData as 1 | 2 | 3 | 4,
    ...
    >
      ...
      {totals.quartersWithData < 4 && (
        ...
        >
          ...
          <Text className="text-xs ml-2 flex-1" style={{ color: '#f59e0b' }}>
            Projection based on {totals.quartersWithData} quarter{totals.quartersWithData > 1 ? 's' : ''} of data.
            Actual annual figures may differ.
          </Text>
        </View>
      )}
      ...
      >
        ...
        <View className="flex-row justify-between py-1">
          <Text className="text-sm" style={{ color: colors.text }}>Total turnover</Text>
          <Text className="text-sm font-bold tabular-nums" style={{ color: colors.text }}>
            {formatGBP(totals.totalTurnover)}
          </Text>
        </View>
        <View className="flex-row justify-between py-1">
          <Text className="text-sm" style={{ color: colors.text }}>Allowable expenses</Text>
        ...
        <View className="flex-row justify-between items-center pt-2 mt-2" style={{ borderTopWidth: 1, borderTopColor: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)' }}>
          <Text className="text-base font-bold" style={{ color: colors.text }}>Net Profit</Text>
          <Text
            className="text-xl font-bold tabular-nums"
            style={{ color: netProfit >= 0 ? '#39AD6A' : '#ee1c1c' }}
          >
            {formatGBP(netProfit)}
          </Text>
        </View>
      ...
      {taxEstimate && (
        <View className="rounded-lg p-4 mb-4" style={{ backgroundColor: isDark ? colors.nav : colors.card }}>
          ...
          <View className="flex-row justify-between py-1">
            <Text className="text-sm" style={{ color: colors.text }}>Taxable profit</Text>
            <Text className="text-sm tabular-nums" style={{ color: colors.text }}>
              {formatGBP(taxEstimate.taxableProfit)}
            </Text>
          </View>
          <View className="flex-row justify-between py-1">
            <Text className="text-sm" style={{ color: colors.text }}>Personal allowance</Text>
            <Text className="text-sm tabular-nums" style={{ color: colors.text }}>
          ...
          {taxEstimate.basicRateTax > 0 && (
            <View className="flex-row justify-between py-1">
              <Text className="text-sm" style={{ color: colors.text }}>Basic rate (20%)</Text>
              <Text className="text-sm tabular-nums" style={{ color: colors.text }}>
          ...
          {taxEstimate.higherRateTax > 0 && (
            <View className="flex-row justify-between py-1">
              <Text className="text-sm" style={{ color: colors.text }}>Higher rate (40%)</Text>
              <Text className="text-sm tabular-nums" style={{ color: colors.text }}>
          ...
          <View className="flex-row justify-between items-center pt-2 mt-2" style={{ borderTopWidth: 1, borderTopColor: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)' }}>
            <Text className="text-sm font-bold" style={{ color: colors.text }}>Total Income Tax</Text>
      ...
      {taxEstimate && (
        <View className="rounded-lg p-4 mb-4" style={{ backgroundColor: isDark ? colors.nav : colors.card }}>
          ...
          {taxEstimate.class2NI > 0 && (
            <View className="flex-row justify-between py-1">
              <Text className="text-sm" style={{ color: colors.text }}>Class 2 (£3.45/week)</Text>
              <Text className="text-sm tabular-nums" style={{ color: colors.text }}>
          ...
          {taxEstimate.ni4LowerBand > 0 && (
            <View className="flex-row justify-between py-1">
              <Text className="text-sm" style={{ color: colors.text }}>Class 4 lower (6%)</Text>
              <Text className="text-sm tabular-nums" style={{ color: colors.text }}>
          ...
          {taxEstimate.ni4UpperBand > 0 && (
            <View className="flex-row justify-between py-1">
              <Text className="text-sm" style={{ color: colors.text }}>Class 4 upper (2%)</Text>
              <Text className="text-sm tabular-nums" style={{ color: colors.text }}>
          ...
          <View className="flex-row justify-between items-center pt-2 mt-2" style={{ borderTopWidth: 1, borderTopColor: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)' }}>
            <Text className="text-sm font-bold" style={{ color: colors.text }}>Total NI</Text>
            <Text className="text-base font-bold tabular-nums" style={{ color: colors.text }}>
      ...
      {taxEstimate && (
        <View
          ...
          style={{ backgroundColor: isDark ? '#312e81' : '#4338ca' }}
        >
      ...
      <View className="rounded-lg p-4 mb-4" style={{ backgroundColor: isDark ? colors.nav : colors.card }}>
        ...
        {quarters.map((q) => (
          <View key={q.quarter} className="flex-row justify-between py-1">
            <Text className="text-sm" style={{ color: colors.text }}>{q.label}</Text>
